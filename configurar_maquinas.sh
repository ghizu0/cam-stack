...
for i in $(seq 1 $num)
do
  sudo docker-machine create -d openstack --openstack-flavor-name medium --openstack-image-name "Ubuntu 20" --openstack-domain-name default --openstack-net-name $red_proyecto --openstack-floatingip-pool ext-net --openstack-ssh-user ubuntu --openstack-sec-groups cam-stack --openstack-keypair-name cam-stack --openstack-private-key-file /home/ubuntu/.ssh/cam-stack.pem nodo$i
done
...
do
  ...
  sudo docker-machine ssh nodo$i "sudo docker volume create --driver vieux/sshfs -o sshcmd=ubuntu@$(ip_privada):/home/ubuntu/volumen -o allow_other -o password=ubuntu volumen-web"
done

echo "Instalando docker-compose en el nodo 1"
sudo docker-machine ssh nodo1 "curl -L \"https://github.com/docker/compose/releases/download/1.29.2/docker-compose-Linux-$x86_64\" -o /usr/local/bin/docker-compose"
